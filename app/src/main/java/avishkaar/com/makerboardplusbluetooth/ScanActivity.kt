...
import kotlinx.android.synthetic.main.activity_scan_activity.*

class ScanActivity : AppCompatActivity(), BluetoothListAdapter.OnDeviceClickListener {
    ...
    override fun onDeviceSelected(bluetoothDevice: BluetoothDevice?) {
        bluetoothScanner?.stopScan(scanCallback)
        val intent = Intent(this,InteractionActivity::class.java).also {
            it.putExtra(Constants.deviceAddress,bluetoothDevice?.address)
        }
